...
router.get('/analytics/dashboard', async (req, res) => {
  try {
    ...
    const inquiries = await Inquiry.find().populate('supplierQuotes.supplierId').lean();
    ...
    for (const inquiry of inquiries) {
      // Get all products for this inquiry from Product schema
      const productIds = inquiry.products.map(p => p.productId?.toString()).filter(Boolean);
      ...
      for (const inquiryProduct of inquiry.products) {
        ...
        const productInDb = products.find(p => 
          p._id.toString() === inquiryProduct.productId?.toString()
        );
        
        // If product not found in database - mark as unavailable
        if (!productInDb) {
          unavailableProducts++;
        ...
        // Check if supplier was sent quote but no response in inquiry
        const wasQuoteSent = productInDb.inquiryId.includes(inquiry.inquiryId);
        
        if (wasQuoteSent) {
          // Quote was sent, check if supplier responded in inquiry
          ...
          if (inquiry.supplierQuotes) {
            inquiry.supplierQuotes.forEach(supplierQuote => {
              const quote = supplierQuote.quotes.find(q => 
                q.productId === inquiryProduct.productId?.toString()
              );
              if (quote) {
                hasSupplierResponse = true;
                const supplierName = supplierQuote.supplierId?.name || 'Unknown Supplier';
                
                // Product is available if price > 0 AND availability is not "no"
                const isAvailable = quote.price > 0 && 
                                 quote.availability && 
                                 quote.availability.toLowerCase() !== 'no' &&
                                 quote.availability.toLowerCase() !== 'out of stock' &&
                                 quote.availability.toLowerCase() !== 'not available';
                ...
                supplierResponses.push({
                  supplierName,
                  price: quote.price || 0,
                  availability: quote.availability || 'Not specified',
                  isAvailable
                });
              }
            });
          ...
          if (!hasSupplierResponse) {
            // Quote was sent but no supplier response
            unavailableProducts++;
            unavailableProductDetails.push({
              inquiryId: inquiry.inquiryId,
              productName: inquiryProduct.name,
              brand: inquiryProduct.brand,
              reason: 'Quote sent to suppliers but no response received'
            });
          } else if (isProductAvailable) {
            // At least one supplier has the product available
            availableProducts++;
          } else {
            // All suppliers responded with price = 0 or availability = "no"
            unavailableProducts++;
            unavailableProductDetails.push({
              inquiryId: inquiry.inquiryId,
              productName: inquiryProduct.name,
              brand: inquiryProduct.brand,
              reason: 'All suppliers reported price = 0 or availability = no',
              supplierResponses: supplierResponses
            });
          }
        } else {
          // No quote was sent to suppliers
          unavailableProducts++;
          unavailableProductDetails.push({
            inquiryId: inquiry.inquiryId,
            productName: inquiryProduct.name,
            brand: inquiryProduct.brand,
            reason: 'No quotes sent to suppliers'
          });
        }
      }
    }
    ...
    res.json({
      ...
      productAnalysis: {
        ...
        availabilityRate: totalProductsRequested > 0 
...
router.get('/analytics/users', async (req, res) => {
  try {
    const users = await User.find({ type: { $in: ['admin', 'company_member'] } })
  ...
  try {
    ...
    const userPerformance = await Promise.all(
      users.map(async (user) => {
        ...
        return {
          ...
          averageActivitiesPerDay: (activities.length / new Date(targetYear, targetMonth + 1, 0).getDate()).toFixed(2)
        };
      })
    );
    ...
    res.json({
      ...
      summary: {
        ...
        totalActivities: userPerformance.reduce((sum, user) => sum + user.totalActivities, 0),
        averageActivitiesPerUser: (userPerformance.reduce((sum, user) => sum + user.totalActivities, 0) / users.length).toFixed(2),
...
router.get('/analytics/export/user-activities', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=user-activities-${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  try {
    ...
    overdueInquiries.forEach(inquiry => {
      notifications.push({
        ...
        message: `Inquiry ${inquiry.inquiryId} from ${inquiry.customerId?.companyName || 'Unknown Company'} has been in processing for over 10 days`,
        priority: 'high',
    ...
    for (const order of oldOrders) {
      const inquiry = await Inquiry.findOne({ inquiryId: order.inquiryId }).populate('customerId');
      if (inquiry) {
        notifications.push({
          title: 'Order Awaiting Response',
          message: `Quote ${order.orderId} sent to ${inquiry.customerId?.companyName || 'Unknown Company'} 10 days ago without response`,
          priority: 'medium',
    ...
    inquiriesWithoutQuotes.forEach(inquiry => {
      notifications.push({
        ...
        message: `Inquiry ${inquiry.inquiryId} from ${inquiry.customerId?.companyName || 'Unknown Company'} has no supplier quotes`,
        priority: 'medium',
        createdAt: inquiry.createdAt,
        type: 'missing_quotes',
        inquiryId: inquiry.inquiryId
...
router.get('/analytics/export-excel', async (req, res) => {
  try {
    ...
    inquiries.forEach(inquiry => {
      inquiry.products.forEach(product => {
        if (inquiry.supplierQuotes && inquiry.supplierQuotes.length > 0) {
          inquiry.supplierQuotes.forEach(supplierQuote => {
            const quote = supplierQuote.quotes.find(q => 
              q.productId === product.productId?.toString()
            );
            ...
            productSheet.addRow({
              ...
              expectedDelivery: quote?.expectedDelivery?.toISOString().split('T')[0] || 'N/A'
            });
          });
        } else {
          productSheet.addRow({
    ...
    overdueInquiries.forEach(inquiry => {
      const daysOverdue = Math.floor((now - inquiry.createdAt) / (1000 * 60 * 60 * 24));
      overdueSheet.addRow({
        type: 'Inquiry',
    ...
    for (const order of overdueOrders) {
      ...
      const daysOverdue = Math.floor((now - order.createdAt) / (1000 * 60 * 60 * 24));
      overdueSheet.addRow({
        type: 'Order',
        id: order.orderId,
        customer: inquiry?.customerId?.companyName || 'N/A',
